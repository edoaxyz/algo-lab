...
\fbox{\parbox{\textwidth}{
        \textbf{Nota sulla complessità computazionale}\newline
        Essendo il grafo $G(V, E)$ non diretto si ha che
        \begin{equation} \label{maxedges}
            \max(|E|) = \frac{n(n-1)}{2} = \Theta(n^2) \text{ con } n = |V|
        \end{equation}
        dunque il massimo numero di archi è minore rispetto al suo equivalente
